...
@task(log_prints=True)
def table_reader(unzip_file_path:Path, input:str, year:int=None) -> pd.DataFrame:
    """
    ...
    desired_columns = {'Page 1 Generation and Fuel Data': json_block.value["sheet_0_columns"] ,
                        'Page 1 Puerto Rico': json_block.value["sheet_1_columns"] ,
                        'Page 1 Energy Storage': json_block.value["sheet_2_columns"] ,
                        'Page 2 Stocks Data': json_block.value["sheet_3_columns"],
                        'Page 2 Oil Stocks Data': json_block.value["sheet_4_columns"],
                        'Page 2 Coal Stocks Data': json_block.value["sheet_5_columns"],
                        'Page 2 Petcoke Stocks Data': json_block.value["sheet_6_columns"],
                        'Page 3 Boiler Fuel Data': json_block.value["sheet_7_columns"],
                        'Page 4 Generator Data': json_block.value["sheet_8_columns"],
                        'Page 5 Fuel Receipts and Costs': json_block.value["sheet_9_columns"],
                        'Page 6 Plant Frame': json_block.value["sheet_10_columns"],
                        'Page 6 Plant Frame Puerto Rico': json_block.value["sheet_11_columns"]                       
                        }
...
def transform(unzip_file_path:Path, year:int) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if len(file_sheets) == 13:
            ...
            df = {'Page 1 Generation and Fuel Data': table_reader(unzip_file_path,'Page 1 Generation and Fuel Data'),
                        ...
                        'Page 2 Stocks Data': table_reader(unzip_file_path, 'Page 2 Stocks Data', year),
...
@flow(log_prints=True)
def main() -> None:
    year = int(String.load(name ="report-year").value)
    ...
    df1,df2,df3,df4,df5,df6,df7,df8,df9,df10 = transform(unzipped_file_path, year)
    local_path = write_local(year,df1,df2,df3,df4,df5,df6,df7,df8,df9,df10)
    write_gcs(local_path)

    if year <= 2022:
        next_year = String(value=year+1)
        next_year.save(name = "report-year", overwrite=True)
